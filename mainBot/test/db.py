...
class TestDB:
    def test_check_user(self):
        ...
        result = DB.check_user_from_db(user_id=TestUser._id)
        assert result == None

    def test_insert_default_user(self, conn, get_time):
        """проверка вставки пользователя в базу данных"""
        time = get_time
        DB.insert_default_user(TestUser._id, *time)
        result = conn.find_one({'_id': TestUser._id})

        assert result != None
        assert result['_id'] == TestUser._id
    ...
    def test_get_user_subs(self, get_time):
        end_user_subs = DB.get_user_subsctription(user_id=TestUser._id)
    ...
    def test_check_user_subsription(self):
        ...
        assert DB.check_user_subscription(user_id=TestUser._id) == False

    """<-------subscriptions------->"""
    
    def test_insert_user_url_in_arr(self, conn):
        """проверка вставки задачи(url) пользователем"""
        DB.insert_user_url_and_title( user_id=TestUser._id, user_url=TestUser._urls[0], user_title=TestUser._titles[0])
        result = conn.find_one({'_id': TestUser._id})

        assert len(result['urls']) == 1
        assert result['urls'][0]['user_url'] == TestUser._urls[0]


    def test_check_count_user_urls(self, conn):
        """проверяем кол-во задач пользователя"""
        check_count_url = DB.check_count_user_url(user_id=TestUser._id)
        assert check_count_url == False

        # заполняем до 5 задач
        for i in range(4):
            DB.insert_user_url_and_title(TestUser._id, TestUser._urls[i+1], TestUser._titles[i+1])
    ...
    def test_check_user_titles(self):
        ...
        for i in range(5):
            assert True == DB.check_user_titles(user_id=TestUser._id, user_title=TestUser._titles[i]) 
        assert False == DB.check_user_titles(user_id=TestUser._id, user_title=dont_title)

    """<-----------end----------->"""
    def test_delete_user(self, conn):
        """удаляем тестового пользователя в базе данных"""
        conn.delete_one({'_id': TestUser._id})
